...
@section('content')
    <div class="row">
        <div class="h1">Nueva publicación</div>
    </div>
    <hr>
                    ...
                    <div class="card-body" style="display:block;">
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea name="descripcion_proyecto" class="form-control" rows="3">{{ old('descripcion_proyecto') }}</textarea>
                            </div>
                                ...
                                <select name="id_area" class="form-control" required>
                                    ...
                                    @foreach ($areas as $a)
                                        <option value="{{ $a->ID_AREA }}"
                                            {{ old('id_area') == $a->ID_AREA ? 'selected' : '' }}>
                                            {{ $a->NOMBRE_AREA }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="id_estado" class="form-control" required>
                                    ...
                                    @foreach ($estados as $e)
                                        <option value="{{ $e->ID_ESTADO }}"
                                            {{ old('id_estado') == $e->ID_ESTADO ? 'selected' : '' }}>
                                            {{ $e->NOMBRE_ESTADO }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="id_autor" class="form-control" required>
                                    ...
                                    @foreach ($investigadores as $i)
                                        <option value="{{ $i->ID_INVESTIGADOR }}"
                                            {{ old('id_autor') == $i->ID_INVESTIGADOR ? 'selected' : '' }}>
                                            {{ $i->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                                <small class="text-muted">Rol (opcional):</small>
                                <input type="text" name="rol_autor" class="form-control" value="{{ old('rol_autor') }}"
                                    maxlength="50">
                            </div>
                                ...
                                <select name="id_grupo" class="form-control">
                                    ...
                                    @foreach ($grupos as $g)
                                        <option value="{{ $g->ID_GRUPO }}"
                                            {{ old('id_grupo') == $g->ID_GRUPO ? 'selected' : '' }}>
                                            {{ $g->NOMBRE_GRUPO }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="coautores[]" class="form-control" multiple size="6">
                                    @foreach ($investigadores as $i)
                                        <option value="{{ $i->ID_INVESTIGADOR }}"
                                            @if (collect(old('coautores', []))->contains($i->ID_INVESTIGADOR)) selected @endif>
                                            {{ $i->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-8 mb-3">
                                <label class="form-label">PDF del artículo (opcional)</label>
                                <input type="file" name="pdf" class="form-control" accept="application/pdf">
                                <small class="text-muted">Sólo PDF, máx. 20 MB.</small>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="row">
                                ...
                                <select name="id_revista" class="form-control" required>
                                    ...
                                    @foreach ($revistas as $r)
                                        <option value="{{ $r->ID_REVISTA }}"
                                            {{ old('id_revista') == $r->ID_REVISTA ? 'selected' : '' }}>
                                            {{ $r->NOMBRE_REVISTA }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" name="anio_publicacion" class="form-control"
                                    value="{{ old('anio_publicacion') }}" min="1900" max="{{ date('Y') + 1 }}"
                                    required>
                            </div>
                        </div>
                    </div>
